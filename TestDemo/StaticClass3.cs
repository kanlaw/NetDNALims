...
namespace TestDemo
{
    public static class StaticClass3
    {
        ...
        {
            StaticClass3.xmppClient = new XmppClient(userName, domain, passWord);
            StaticClass3.xmppClient.Hostname = server;

            //StaticClass.xmppClient.Open();

            StaticClass3.pm = new PresenceManager(xmppClient);
            StaticClass3.rm = new RosterManager(xmppClient);

            StaticClass3.muc = new MucManager(xmppClient);

            StaticClass3.fm = new FileTransferManager();
            //StaticClass.fm.OnProgress += Fm_OnProgress;
            //StaticClass.fm.OnEnd += Fm_OnEnd;

            return StaticClass3.xmppClient;
        }
